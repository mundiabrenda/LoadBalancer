...
def hash_virtual_server(i, j):
    return (i + j + 2*j**2 + 25) % num_slots
...
for server_id in range(N):
    for j in range(K):
        ...
        consistent_hash_map[virtual_server_id] = server_id

#Step 3
def map_request(request_id):
    hashed_request = hash_request(request_id)
    ...
    server_id = consistent_hash_map[next_virtual_server_id]
